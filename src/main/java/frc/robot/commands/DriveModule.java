 ...
 */
public class DriveModule extends CommandBase {
    // Variáveis que guardam nossas funções do gamepad
    DoubleSupplier y;
    DoubleSupplier x;
    DoubleSupplier turn;
    DoubleSupplier module;
    ...
    public DriveModule(SwerveSubsystem swerve, DoubleSupplier y, DoubleSupplier x, DoubleSupplier turn, DoubleSupplier module) {
    ...
    @Override
    public void execute() {
      // Obtemos os valores do controle
      double yM   = y.getAsDouble();
      double xM  = x.getAsDouble();
      double angVelocity = turn.getAsDouble() * Tracao.multiplicadorRotacional;
      double modulo = module.getAsDouble();

      // Obtemos a translação do robô
      translation = new Translation2d(yM * Tracao.MAX_SPEED * modulo, xM * Tracao.MAX_SPEED * modulo);
